---
title: "W3 practical: example ChIPseq"
...
We will perform the primary analysis of an example dataset, at each step demonstrating **both the use of both a very standard pipeline, and a full R-based equivalent**. Pay attention to which chunks are R or bash code (see header)
...
```{r}
head(peaks)
plotSignalTracks(c(Myc="aligned/Myc.bam", Input="aligned/input.bam"), region=peaks[1],
                 extend=1000)
```
