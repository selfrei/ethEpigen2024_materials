...
sm <- signal2Matrix(bwfiles, regions=pp, w=50)
...
sm2 <- renormalizeSignalMatrices(sm2, method="top")
plotEnrichedHeatmaps(sm2, cluster_rows = "sort", scale_rows = TRUE, colors = c("white","darkblue"), use_raster=TRUE)
plotEnrichedHeatmaps(sm[,6])
```

Looking at a larger interval around p300 peaks:

```{r}
sm5kb <- signal2Matrix(bwfiles[c(3,5)], regions=peaks$p300, w=100, extend = 5000)
